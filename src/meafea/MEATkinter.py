...
condition_dict={'Well Label':mtk.selected_well.get(), 'Experiment':mtk.selected_experiment.get(),
                        mtk.comparevar:mtk.dose_var.get()}

mtk.spikedata[mtk.spikedata['Experiment']=='iN47CNVDelDIV48Rotenone1uMhr0']['Dose Label'].unique()


mtk.wellspikes.shape


mtk.sourcerawinteractive
mtk.wellnetworkburstdata
...
nameoftheobject='ah301024detwithmad5fromcontrol_iN47_pharm'
mymea=load_object(os.path.join(dpath, nameoftheobject+'.pkl'))
# We could save that as files in the future analogous to save spike and burst data.
if mymea.networkburstdata.shape[0]!=0:
    mymea.More_NetworkBurstFeatures()

    mymea.networkburstdata.rename(columns=mymea.column_rename_networkburstdata, inplace=True)
...
mtk= MEATkinterApp(mymea, 'Dose Label')
...
mtk.wellspikes['Channel Label'].unique()

mtk.wellspikes['True Label']
